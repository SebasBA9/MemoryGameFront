describe('template spec', () => {
  it('Validación de la primera vista ', () => {
    cy.visit('http://localhost:8080/')
    //Texto del titulo correcto
    cy.get('#Title').should('have.text','¿Eres el mejor maestro pokemon del mundo?')
    //Texto del subtitulo correcto
    cy.get('#Subtitle').should('have.text','Memoriza la mayor cantidad de Pokemons y demuestralo!!')
    //Texto del subtitulo correcto
    cy.get('#Second_Subtitle').should('have.text','Equipo elegido para esta ronda:')
    //Se muestra boton 
    cy.get('#GameButton').should('be.visible');
    //simula click del boton
    cy.get('#GameButton').click();
    //muestra las primeras instrucciones 
    cy.get('#Instructions').should('have.text','Para enviar la secuencia de Pokemons da click en el Pokemon deseado, una vez que termines tu secuencia enviala dando click en la Pokebola')
    //muestra las segundas instrucciones 
    cy.get('#SecondInstructions').should('have.text','Si te equivocas no pasa nada, solo da click en el pokemon que quieras remover y este será eliminado de la secuencia')
  ...
  it('La  prueba  deberá  interceptar  la  llamada  realizada  por  el  botón', () => {
    ...
    // Intercepta la peticion de llamar secuencia
...
describe('Creación y envío de secuencia', () => {
  ...
  it('Se añade el pokemon seleccionado a la secuencia del jugador', () => {
  ...
  it('La prueba debe verificar que el botón de "Enviar Secuencia" solo aparezca cuando el número de Pokemons en la secuencia a memorizar coincida con el de la secuencia a enviar. ', () => {
    // Espera a que el equipo Pokemon esté cargado
    cy.get('.button-container').should('not.contain', 'Cargando equipo Pokemon...');
  ...
  describe('Finalización del juego', () => {
    ...
    it('La  prueba  debe  de  validar  que  al  finalizar  el  juego  aparezca  el  número  de pokemons que el jugador memorizó', () => {
      ...
      cy.wait('@enviarSecuencia').then(({ response }) => {
        ...
        cy.get('#Score').should('contain.text', 'Puntaje: 0')
