<template>
  <div id="app">
    ...
    <h2 v-if="!juegoIniciado" id="Title">¿Eres el mejor maestro pokemon del mundo?</h2>
    <h3 v-if="!juegoIniciado" id="Subtitle">Memoriza la mayor cantidad de Pokemons y demuestralo!!</h3>

    <div v-if="!juegoFinalizado">
      <h1 v-if="!juegoIniciado" id="Second_Subtitle">Equipo elegido para esta ronda:</h1>
      <h1 v-else>Instrucciones:</h1>
    </div>

    <p v-if="juegoIniciado && !juegoFinalizado" id="Instructions">Para enviar la secuencia de Pokemons da click en el Pokemon deseado, una vez que termines tu secuencia enviala dando click en la Pokebola</p>
    <p v-if="juegoIniciado && !juegoFinalizado" id="SecondInstructions">Si te equivocas no pasa nada, solo da click en el pokemon que quieras remover y este será eliminado de la secuencia</p>
    ...
    <br v-if="!juegoIniciado">
    <button id="GameButton" class="start-button"
